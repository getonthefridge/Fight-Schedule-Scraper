...
#         raw_country = country_text_div.get_text(strip=True)
#         if raw_country:
#             # Map UK regions that coco doesn't recognize
#             UK_REGIONS = {"Scotland", "Wales", "England", "Northern Ireland"}
#             if raw_country in UK_REGIONS:
#                 country = "GBR"
#             else:
...
# ):
#     if not fights:
#         print(f"⚠️  No fights found for {event_url} — skipping.")
#         return []
#     store = _get_authorized_store()
#     cal = _get_calendar(store, calendar_name)
#
...
#
#         # Build notes
#         notes_lines = [f"🔗 {event_url}", f"FightID: {unique_tag}\n"]
#         # notes_lines = [f'{event_url}\n']
#         for fight in section_fights:
#             rank_r = f" {fight.red_corner.rank}" if fight.red_corner.rank else ""
#             rank_b = f" {fight.blue_corner.rank}" if fight.blue_corner.rank else ""
#             notes_lines.append(
...
#
#         notes = "\n".join(notes_lines)
#         notes += f'\nPublished on {datetime.now().strftime("%A, %B %d, %Y at %I:%M %p")}'
#         title = f"{base_title}  |  {section_name}"
#
#         # ── Search for an existing event with this unique tag ──
#         existing_event = None
